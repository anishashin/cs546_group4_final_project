...
router.post('/', async (req, res) => {
    ...
    savedPlateInfo.userId = xss(savedPlateInfo.userId);
    savedPlateInfo.title = xss(savedPlateInfo.title);
    if(!savedPlateInfo) {
        res.status(400).render('error', {
            authenticated: req.session.user ? true : false,
            user: req.session.user,
            title: 'Error', 
            error: 'You must provide data to create a saved plate.'
        });
        return;
    }
    ...
    if(!savedPlateInfo.title || typeof savedPlateInfo.title !== 'string' || savedPlateInfo.title.trim() === '') {
        res.status(400).render('error', {
            ...
            user: req.session.user,
            title: 'Error', 
            error: 'Title must be a non-empty string containing more than just spaces.'
        });
        return;
    }
    ...
    for(let i = 0; i < savedPlateInfo.foods.length; i++) {
        if(!savedPlateInfo.foods[i] || typeof savedPlateInfo.foods[i] !== 'string' || savedPlateInfo.foods[i].trim() === '') {
            res.status(400).render('error', {
                ...
                user: req.session.user,
                title: 'Error', 
                error: 'Foods must only contain non-empty string(s) with more than just spaces.'
            });
            return;
        }
        savedPlateInfo.foods[i] = xss(savedPlateInfo.foods[i]);
    }
    try {
        for(let i = 0; i < savedPlateInfo.foods.length; i++) {
    ...
    if(Array.isArray(savedPlateInfo.servings) === false || savedPlateInfo.servings.length !== savedPlateInfo.foods.length) {
        res.status(400).render('error', {
            ...
            user: req.session.user,
            title: 'Error', 
            error: 'Servings must be an array with the same length as foods.'
        });
        return;
    }
    for(let i = 0; i < savedPlateInfo.servings.length; i++) {
        if(!savedPlateInfo.servings[i] || typeof savedPlateInfo.servings[i] !== 'number' || savedPlateInfo.servings[i] <= 0) {
            res.status(400).render('error', {
                ...
                user: req.session.user,
                title: 'Error', 
                error: 'Servings must only contain numbers greater than 0.'
            });
            return;
        }
...
router.put('/:id', async (req, res) => {
    ...
    if(!savedPlateInfo) {
        res.status(400).render('error', {
            ...
            user: req.session.user,
            title: 'Error', 
            error: 'You must provide data to edit a saved plate.'
        });
        return;
    }
    if(!savedPlateInfo.title || typeof savedPlateInfo.title !== 'string' || savedPlateInfo.title.trim() === '') {
